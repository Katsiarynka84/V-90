...
tickets = [
    ...
    ('Mark', 'b', BUSINESS_COST, 4),
    ('Mila', 'b', BUSINESS_COST, 6),
    ('Tim', 'e', ECONOMY_COST, 44),
    ('Tom', 'e', ECONOMY_COST, 88),
    ('Ivan', 'b', BUSINESS_COST, 9),
    ('Kate', 'e', ECONOMY_COST, 55)
]
...
while True:
    ...
    if commandlet == '1':
        ...
        if not prev_seat.isdigit() or int(prev_seat) not in [ticket[3] for ticket in tickets]:
            print(f'Некорректный номер: {prev_seat}')

        else:
            for i in range(len(tickets)):
                if (int(prev_seat)) == tickets[i][3]:
                    print(tickets[i])  # старый билет из списка
                    if tickets[i][1] == 'e':  # если класс Экономи
                    ...
                    if not new_seat.isdigit() \
                            or int(new_seat) in [ticket[3] for ticket in tickets] \
                            or int(new_seat) not in numbers_for_choose:
                        print(f'Место занято либо введён некорректный номер: {new_seat}')

                    else:
                        new_ticket = (tickets[i][0], tickets[i][1], tickets[i][2] + SEAT_CHANGE_COST, int(new_seat))
                        del tickets[i]
                        tickets += [new_ticket]
                        print(f'Билет добавлен! {new_ticket}')
                    break
        ...
        total_cost = sum([ticket[2] for ticket in tickets])
